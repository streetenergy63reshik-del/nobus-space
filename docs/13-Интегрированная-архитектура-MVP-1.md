...
**Статус реализации:** NOT CURRENT; Gate 0–8, включая обязательный Gate 2A, не считаются пройденными

**Исследовательская база:** `9d816b35d3f419b42e24ad09ae6aadc92c33db43`

**Дата:** 28 июля 2026 года

**Назначение:** связать архитектуры Gate 0–8, включая Gate 2A, в одну систему, определить
владельца каждого контракта, разрешить межблочные противоречия и не допустить
создания второго бота, backend, effect engine или параллельного agent framework.
...
последовательно приводятся к одной модели контрактов, одному Agent Registry и
одному Core-owned effect plane. Mini App является новым интерфейсом этого Core.
...
- показывает те же task/plan/approval/evidence/artifact states в owner-only Mini App;
- маршрутизирует глубокую работу закрытому specialist worker, сохраняя authority в Core;
- принимает запрос на разработку и возвращает проверенный local candidate без self-deploy.
...
flowchart LR
    ...
    O --> U["Owner-only Telegram Mini App"]
    U --> API["Same-origin Control API"]
    API --> C
    ...
    C --> AR["Closed Agent Registry / AgentDispatch"]
    C --> Q["Atomic task/job/effect admission"]
    AR --> S1["General orchestrator worker"]
    AR --> S2["Google Workspace specialist"]
    AR --> S3["Research & Analytics specialist"]
    AR --> S4["Content Studio specialist"]
    AR --> S5["Development specialist"]
    AR --> S6["Verification specialist"]
    Q --> B["Business Notes"]
    Q --> G["Official Google adapters"]
    Q --> W["Windows Document Bridge closed jobs"]
    Q --> DW["Windows Development Worker closed jobs"]
...
approval, idempotency, task/queue/effect state, Agent Registry и reconciliation.
Specialist workers не вызывают друг друга напрямую: Core создаёт typed
`AgentDispatch`, проверяет closed `WorkerResult` и решает следующий шаг. Codex —
основной planner/developer, Gemini — bounded Google specialist. Ни один worker не
получает OAuth, raw filesystem или право исполнять внешние эффекты.

## 4. Владение контрактами

| Контракт или ответственность | Единственный владелец | Потребители |
|---|---|---|
| Baseline Evidence Pack, Product Contract, corpus | Gate 0 | Gate 1–8, включая 2A, release |
| `SemanticProposal`, `IntentEnvelope`, clarification frame | Gate 1 | Core/router/policy |
| Scope/deny/output/repository registries и document/code contracts | Gate 2 | Gate 2A/3/5/6/7 |
| Agent Registry, Control API/Mini App, generic task/effect lifecycle, Development Worker/code candidate | Gate 2A | Gate 3–8 |
| Google identity, OAuth, REST adapters, provider budget и Workspace specialist profile | Gate 3 | Gate 4/5/6/7 |
| Notes/Calendar/Tasks payloads, provider outcomes и reconciliation поверх Gate 2A effect plane | Gate 4 | Business write adapters |
| Document Bridge protocol, opaque local identity, bounded parsers | Gate 5 | Gate 6/7/8 |
...
| Final integrated release manifest, global fencing, health, backup/pilot evidence | Gate 8 | Operations |
...
3. Поиск выполняется metadata-first. Для Drive-wide поиска отдельная discovery
   capability имеет только metadata authority и не считается source scope.
4. При одном точном кандидате или выборе владельца Core создаёт exact
   project/client-bound file/folder registry/ref и только затем `DocumentRef`;
   broad OAuth grant никогда не подменяет эту привязку.
...
- FastAPI + loopback Uvicorn за HTTPS edge для Mini App/Control API;
- buildless static HTML/CSS/ES modules, SSE с bounded polling fallback;
- server-side Telegram `initData` validation и короткие in-memory sessions;
- official Google Workspace REST;
- `google-genai`/Vertex AI как основной Google-specialist path;
- persistent официальный Codex app-server/SDK из поставки Codex CLI как
  основной CLI-first worker path; одноразовый `codex exec` только bounded
  fallback, OpenAI API — только после отдельного cost/reliability decision;
...
- внешний Healthchecks.io heartbeat с Gmail owner alert, но не как единственный
  источник health evidence.
...
Linux VPS
  HTTPS edge :443
    ...
    -> same-origin Control API / SSE
  systemd
    -> one immutable Nobus Core release
    -> one globally fenced Telegram poller
    -> policy / Agent Registry / queue / effect / reconciliation
    -> Google adapters / Codex coordination / Gemini gateway
    -> dynamically inventoried SQLite DB set

Windows PC
  WinSW
    -> one authorized Development Worker from Gate 2A
       -> registered worktrees / Codex candidate jobs only
       -> dedicated non-exportable device identity
    -> one separate Document Bridge from Gate 5
       -> closed local-library operations only
       -> separate non-exportable device identity
...
Independent alert
  content-free outbound heartbeat
    -> Healthchecks.io
    -> owner Gmail
```
...
- Core, Development Worker и Document Bridge identity/generation leases;
- Mini App authentication/session/API/SSE readiness;
- Agent Registry manifest equality и отсутствие orphan `AgentDispatch`;
- Google adapter identity/budget/readiness;
- Codex worker auth/entitlement, app-server generation, session probe,
  bounded-turn freshness и circuit state;
- exact manifest-bound set каждого release-required Google model route:
  set equality/uniqueness, auth, budget reservation/settlement, fixed PUBLIC
  synthetic sentinel freshness и circuit state;
...
| Mini App auth/session invalid | Fail closed; task/effect не создаётся |
| Development Worker offline | Development DEGRADED; остальные routes продолжают работу |
| Specialist result/schema invalid | Result rejected; no downstream authority/effect |
...
- repository/code scope registries для Gate 2A;
- opaque references;
- strict seven-contract family;
- policy before content/model.

### Gate 2A — полноценный Mini App и Development Control

- первый bounded Linux Core/HTTPS release, но не финальный MVP release;
- owner-only Mini App и same-origin Control API;
- Agent Registry/AgentDispatch и generic durable task/effect plane;
- один outbound Windows Development Worker;
- Telegram/Mini App → Codex → isolated worktree → tests/evidence → local candidate;
- никакого terminal/Web IDE, remote/push/deploy или self-update.

### Gate 3 — официальный Google foundation

- Web OAuth/token broker и account binding;
- official REST adapters;
- Google Workspace specialist без OAuth/effect authority и bounded Vertex/google-genai gateway;
- read-only canary отдельно от production executor.

### Gate 4 — business effects поверх единого effect plane

- применить Gate 2A atomic admission effect+job;
- расширить единый lifecycle/outcome, не создавая второй engine;
- Calendar/Tasks/Notes exact contracts;
- unknown/reconciliation/orphan recovery.

### Gate 5 — тонкий Windows Bridge

- переиспользовать Gate 2A authenticated outbound transport/fencing library;
- отдельная service identity от Development Worker;
...
- Research & Analytics specialist возвращает один closed `AnalysisResult`;
...
- Content Studio specialist потребляет только verified `AnalysisResult` и не пересчитывает факты;
...
- финальная интеграция уже принятого Gate 2A server/Mini App foundation;
- immutable Core/Development Worker/Document Bridge artifacts;
- one global poller;
...
3. Mini App и Telegram используют один Core/state/effect plane;
4. specialist workers не имеют peer authority, credentials или внешних effects;
5. local path и provider IDs не являются model authority;
6. атомарность admission и provider-outcome/outbox определена;
7. provider unknown отделён от delivery unknown;
8. real CAS заявляется только для backend, где он доказан;
9. renderer не пересчитывает факты;
10. runtime topology содержит один Core/poller и отдельные bounded worker identities;
11. rollback учитывает внешние effects;
12. CURRENT нигде не выводится из архитектурного текста.

Фактический MVP-1 PASS остаётся возможен только после реализации и принятия
Gate 0–8, включая обязательный Gate 2A, по [`docs/12`](12-Эталон-MVP-1-и-дорожная-карта.md).
