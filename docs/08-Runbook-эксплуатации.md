...
Сохранён квалификационный профиль: `NobusSpaceBot` остановлен, logon trigger отключён и без автоматических повторов; `NobusSpaceBot-Health` отключён; `NobusSpaceBot-Backup` отключён после успешной реальной проверки. В расписании задано 03:30 МСК, но при Disabled следующего автоматического копирования нет.
