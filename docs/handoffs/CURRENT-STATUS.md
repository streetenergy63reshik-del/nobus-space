...
**9 сентября 2026. C6: исправлена композиция внешнего хранилища; MVP1 NOT READY.** Источник `1bb1d85b9f9d6c7f8c5ed1edbf5e69680a021921` сохраняет отдельные временные файлы каждого state внутри worktree. Настоящая проверка build_worker_env больше не отвергает допустимый профиль. Пройдена 201 затронутая проверка; независимые L2/L3 приняли точные bytes. [Доказательства](../gates/gate-c6-release/SOURCE-CORRECTION-03.json).

Исправление stdin опубликовано обычным [PR23](https://github.com/streetenergy63reshik-del/nobus-space/pull/23), merge `4c6d400898a47c63a993171f57d8be4265b3eec8`. В рамках прямого разрешения владельца на все необходимые действия для завершения MVP1 применён C6-ACT-01-D3. LIVE и конфигурация переключены, новая резервная копия расшифрована и проверена. Production ещё не запускался; три задания Disabled.

Recovery-c6-03 прошёл прежнюю точку зависания, но остановился до worker/ASR из-за неверного temp path. Все строки recovery и production, checkpoint и canonical20 WIP сохранены; порт и mutex свободны. Неудачные попытки и расход бюджета сохранены. Полный RTO пока не засчитан.

Следующие действия: публикация новой проверенной коррекции, точный D4 план и применение в рамках данного разрешения, полный RTO≤30мин, production/backup/restart и реальные owner text/voice/Mini App/download сценарии. После ≥15мин наблюдения и явной пользовательской приёмки — постоянная фаза и v1.0.2. [C6 handoff](../gates/gate-c6-release/HANDOFF.md). Памятка DRAFT; MVP2 HOLD.
