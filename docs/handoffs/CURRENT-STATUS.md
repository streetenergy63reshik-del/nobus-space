...
**9 сентября 2026. C6: второе исправление запуска проверено; MVP1 NOT READY.** Источник `bba7d21530bd467339d1982b46051e578408f66d` изолирует stdin запроса Git revision от ожидающего потока остановки. Настоящий Windows regression сначала воспроизвёл зависание; после исправления пройдено127 затронутых проверок и независимые L2/L3. [Привязка проверки](../gates/gate-c6-release/SOURCE-CORRECTION-02.json).

Предыдущее исправление PROGRAMDATA опубликовано обычным [PR22](https://github.com/streetenergy63reshik-del/nobus-space/pull/22), merge `0b6425fed9aa82417cba299352d058d55811109c`. Владелец подтвердил точное дополнение C6-ACT-01-D2; LIVE и конфигурация переключены, свежая зашифрованная копия production state проверена. Все три задания Disabled; production ещё не запускался.

Вторая попытка отдельного recovery drill остановилась до worker/ASR/readiness из-за наследования stdin запросом Git. После остановки все строки recovery и production, checkpoint и canonical20 WIP неизменны; порт и mutex свободны. Обе неудачные попытки и общий бюджет сохранены. Полный RTO не засчитан.

Остаются публикация исправления и подтверждение точной дельты активации, полный RTO≤30мин, Bot/Mini App, настоящий scheduled backup и controlled restart, пользовательские text/voice/download сценарии, ≥15мин наблюдения и явная итоговая приёмка. Затем постоянная активация и v1.0.2. Памятка пока DRAFT; MVP2 HOLD. [C6 handoff](../gates/gate-c6-release/HANDOFF.md).
