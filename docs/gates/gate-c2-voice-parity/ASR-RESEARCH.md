...
По отдельному согласию владельца выполнена одна проба beam_size8→1.
Все остальные настройки и входные bytes сохранены. Результат dev16:
WER8,85%, CER2,16%, critical2, raw RTFp95 0,581>0,5; latency p95 7,022s
вместо14,002s. Ошибки critical относятся к cancel/domain; улучшение скорости
не компенсирует качество. Cold4,819s, RAM2922139648B, Job active0.
Вторая проба заняла50,630924s; small суммарно129,151549s из1200s,
остаток1070,848451s. **Hard FAIL; дальнейшие конфигурации остановлены.**
Всего семь не прошедших вариантов двух семейств. Holdout остаётся закрытым.

Следующая независимая гипотеза — Google Chirp3 ru-RU, eu, synchronous
Recognize на тех же16 dev samples. Их общая длительность156,26s, максимум
одного файла43,19s, поэтому короткий synchronous route применим без GCS.
Google документирует ru-RU/eu как GA и поддержку Recognize.
[Модель и методы](https://docs.cloud.google.com/speech-to-text/docs/models/chirp-3),
[inline audio](https://docs.cloud.google.com/speech-to-text/v2/docs/sync-recognize).
Это основание подготовить сравнение, а не обещание critical0 или PASS.

Предлагаемая проба:16 запросов без автоматических повторов, до10 минут,
предел API cost0,10USD, только те же синтетические WAV, без GCS и создания
ресурсов. Опубликованный V2 standard тариф0,016USD/мин даёт ориентир около
0,05USD; условия конкретного billing account ещё не проверены.
[Pricing](https://cloud.google.com/speech-to-text/pricing).
Нужен owner-selected существующий Google Cloud project с доступным Speech-to-Text
и billing; штатные auth/region/logging и V2 retention должны быть проверены
до точной авторизации trial. V1 FAQ не переносится на V2 как доказательство.
Ни подготовка этой гипотезы, ни прежние local/ChatGPT разрешения не разрешают
облачную передачу аудио. Облачных вызовов по-прежнему0.
