...
**20 сентября 2026:** после отдельного инцидента развёрнут `9065cfc` (PR №38), реализующий [ADR 0028](adr/0028-post-mvp1-power-transition-recovery.md). Backup/start завершён, readiness, сохранность четырёх БД и два Health PASS проверены до 16:29 МСК. Факты и ограничения — в [записи инцидента](incidents/2026-09-20-production-stop.md) и [CURRENT](handoffs/CURRENT-STATUS.md).
...
При `recovery_history_blocked` после выключения Windows сверить BootIdentifier, подлинную историю, четыре БД, эффекты и доставки. Прежняя версия `0c9b778` требовала иной BootIdentifier; действующий `9065cfc` поддерживает описанную ниже узкую альтернативу. Если доказательство отсутствует, нужна операторская сверка с точным digest, а не изменение заданий или удаление истории. Если main/health отключены, а backup включён, `inspect-recovery` вернёт `activation_binding_invalid`: для режима с отключёнными заданиями все три должны быть отключены. Перед временным отключением backup проверить, что он не работает и ближайший запуск не наступит во время операции; затем восстановить расписание через штатный оператор и readback.

В действующей версии с [ADR 0028](adr/0028-post-mvp1-power-transition-recovery.md) тот же BootIdentifier допускает согласование лишь при свежей паре событий Windows System, сохранном подписанном `starting`, повторной проверке события и полном отсутствии прежнего runtime, активной lease, неизвестных эффектов и доставок. Переход старше 15 минут или ошибка чтения остаются STOP. В истории будет `power_reconciled` v5. Старый код после такой записи нельзя запускать без двух проверенных rollback reader-патчей на точной базе `0bd63db`; к `0c9b778` они напрямую не применяются. Настройка Fast Startup на этом ПК осталась включённой: изменение `HiberbootEnabled` отклонено правами Windows.

Одинаковое `kind=action` хранит Telegram callback и capability исполняемого эффекта. В `0c9b778` истёкшая кнопка могла блокировать boot reconciliation. Действующий валидатор допускает только подлинный, истёкший owner callback одной из четырёх голосовых команд MVP1; действующая или расширенная кнопка, неверная привязка и любой незавершённый эффект оставляют STOP. Не удалять запись вручную и не считать её неизвестным внешним эффектом без проверки типа, привязки, срока, очереди и доставок.
...
При вызове `managed_backups.create_generation` оператор не должен заранее захватывать Core mutex: штатный `backup_telegram_runtime.backup` захватывает его сам. Повторный захват даёт `RunnerAlreadyActive` и может оставить только подписанный intent поколения. После такого отказа сначала доказать STOP, точный attempt и состав частичного поколения, затем использовать `recover_partial` для сохранения в карантине. Не повторять использованный intent и не удалять каталог вручную.
