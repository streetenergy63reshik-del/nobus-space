...
Text и подтверждённый voice ingress соединены с Core, SQLite и outbox локальной fake-композицией. Terminal Task/event/outbox transitions атомарны; restart не повторяет незавершённый worker автоматически. Authenticated network identity boundary, live CLI worker и production deployment отсутствуют.
...
| Voice service | принять ограниченные bytes, transcribe, выдать и единожды подтвердить actor-bound preview | provider недоверен; confirmation boundary доверенный | preview и in-memory confirmation wired в local runtime |
| Prompt enricher | предложить instruction/criteria | недоверенный LLM worker | отсутствует |
| Local durable vertical | доказать text/voice → Core → SQLite → fake worker → L1/L2/L3 → outbox | демонстрационный | интегрирован, без network/live worker |
