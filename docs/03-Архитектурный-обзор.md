...
- Telegram parser/gateway и bytes-only voice preview;
...
- изолированный stdlib SQLite-модуль для durable ingress/task checkpoints и ordered events; runtime wiring отсутствует.

Text ingress и CLI boundary соединены с Core только локальной fake-композицией. SQLite checkpoint-модуль ещё не подключён к ней. Voice confirm flow, authenticated network identity boundary, live CLI worker и production deployment отсутствуют.
...
| Voice service | принять ограниченные bytes, temp file, transcribe, очистить | provider недоверен | интегрирован, bytes-only preview |
...
## 11. Voice preview (TARGET; CURRENT service — bytes-only)
