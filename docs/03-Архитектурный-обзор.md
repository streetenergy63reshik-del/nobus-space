...
- отдельный server-owned `owner.library.read` для `C:\Хранилище\АГЕНТ`: только в read-only worker, несовместим с `repo.write`; Git worktree остаётся единственной границей code effect;
