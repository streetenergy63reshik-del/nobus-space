...
**14 сентября 2026, 18:47 МСК:** MVP1 `82003c03` принят, v1.0.2 опубликован, но текущий runtime DOWN после `runtime_failed` в 14:39:09 МСК. Main enabled/Ready/1; exact LIVE supervisor/Core/relay/listener/lease нет, local/public FAIL. Revisions `73ed1c9`, `19c9bf3`, `121a4df` и `e357629` отклонены; protocol v3 остаётся локальным WIP до нового freeze, real fixture и независимых L1/L2/L3. [Текущий статус](handoffs/CURRENT-STATUS.md) и [пакет M1-S1](gates/mvp1-maintenance/HANDOFF.md) содержат привязанные факты.
...
В принятом v1.0.2 Health не создаёт второй runtime, а legacy Scheduler retries не считаются recovery. Текущий maintenance WIP переносит ограниченные повторы внутрь одного Scheduler action: максимум 10 retries через 60 секунд, только для exact allowlisted transient outcome и после proven cleanup; permanent, `UNKNOWN`, invalid/missing или противоречащий exit code Core outcome, повреждённая evidence history или exhaustion дают typed STOP. Четыре frozen revisions отклонены. До PASS real three-case Scheduler fixture, L1/L2/L3, publication и activation это TARGET, а не действующее поведение.
...
Production-задачи заменяются только как проверяемая транзакция. Сначала экспортируются exact XML main/health/backup и hashes локальных config. После штатной остановки и доказанного отсутствия exact процессов/Job/lease/listener отключаются только эти три задачи. Backup installer принимает `-ReplaceExisting` лишь для уже остановленной и disabled exact task; затем ставятся main/health и весь профиль читается обратно. При промежуточном отказе admission остаётся закрытым и runtime не запускается. Возврат прежних code/config/task definitions допускается только из экспортированных exact bytes по отдельному решению; состояние БД на старую копию не откатывается.
