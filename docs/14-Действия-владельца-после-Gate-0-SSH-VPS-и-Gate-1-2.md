...
Продуктовый checkpoint C2 — `28222923d2c2560504d1237c8e8da6df442c6150`,
tree `b24ae81b91786aca942c89a8d54f50279de3cef8`. Владелец 5 сентября принял
...
Small использовал 858.434166 из 1200 s; GigaAM 194.916977 из 1800 s. Новые модели,
установки, cloud ASR, live/config и C3 не разрешены этим продолжением.

Дополнительная сессия B02, разрешённая 6 сентября, дала 5 готовых ответов с независимыми
25/25; выполнено 19/20 calls. Основная text/voice-пара, correction, negation и voice
restart/replay пройдены. UNKNOWN text/voice безопасно отклонены, требуемое решение Core
не получено. Старые FAIL сохранены; пять ответов не равны PASS всей матрицы.

Текущее недостающее внешнее разрешение — один узкий UNKNOWN trial: до 8 turn,
включая повторы, и 600 s от первого вызова (план 4+резерв 4). Та же подписка и
gpt-5.6-sol/high/fast, стандартный endpoint; только синтетический текст, без audio,
tools/effects/API billing/покупок; имеющаяся квота расходуется. Жёсткого token ceiling
нет, region/retention неизвестны. [Точный план](gates/gate-c2-voice-parity/PRODUCT-TRIAL-PLAN.md).
