...
Принятая база C1: 43e753c571e1ad8db5af5f453b5db0c0b417cac8, tree a7c6328a42412a0bd269004aefa9c9d402c75564. C1 повторно не принимался. C3 начинается только отдельной задачей по запуску владельца после публикации C2. Exact опубликованный commit/tree и PR будут записаны в публикационном разделе ниже; собственный SHA этого документа определяется через Git, а не самоссылкой внутри его bytes.
...
PUBLICATION PENDING. Разрешены обычные branch push, PR в protected main и merge после сохранённого PASS, без force/protection bypass. Точный manifest готовится из текущего HEAD; source/tests/model/criteria должны совпасть с принятым freeze. Документная фиксация результата merge не запускает продуктовую кампанию заново.
