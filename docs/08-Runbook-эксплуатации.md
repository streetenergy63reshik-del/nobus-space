...
При подтверждении голосового preview callback немедленно получает краткий Telegram toast `Обрабатываю…`; отдельное сообщение в чат не отправляется. Это подтверждает приём кнопки, но не сокращает worker contract: окончательный ответ Codex по-прежнему может занять до 120 секунд.
