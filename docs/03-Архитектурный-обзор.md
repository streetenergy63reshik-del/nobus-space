...
Text и подтверждённый voice ingress соединены с Core, SQLite и outbox локальной fake-композицией. Terminal Task/event/outbox transitions атомарны; restart не повторяет незавершённый worker автоматически. Owner-bound authenticated Telegram polling поддерживает безопасные control-команды. Confirmed fake-task flow воспроизведён live через owner network `/task` → `/confirm` → terminal `completed`; worker при этом оставался локальным fake. Voice network path, live CLI worker и production deployment отсутствуют.
...
|---|---|---|---|
| Telegram transport gateway | разобрать update, нормализовать text/voice/callback | недоверенный parser | live owner-bound status control и confirmed fake-task E2E; voice/callback path не активирован |
