# Действующий протокол C2: подтверждаемый голосовой ввод

Решение владельца от **5 сентября 2026** принято в этой же задаче явным поручением выполнить GATE-C2-CLOSURE-PROMPT. Машиночитаемый контракт — [PROTOCOL.json](PROTOCOL.json), версия `c2-confirmed-voice-3.0.0`; scorer/review версия3.0.0. Нормализация и gold не меняются. Редакция фиксируется до открытия holdout.

Владелец изменил ровно два task-level условия: буквальный critical-token zero теперь диагностика, а aggregate raw RTFp95≤0.5 не отдельный blocker. Причина — оценка подтверждаемого ввода с сохранением смысловых фактов и исходного времени ожидания каждого файла `max(5s,0.5×duration)`. Старые FAIL по прежнему протоколу сохраняются и не превращаются в PASS. Нижеследующий дизайн4 сентября сохранён как история первоначального протокола; только эти два его hard условия заменены текущим явным решением.

До holdout независимо оцениваются уже сохранённые dev outputs CURRENT base/GigaAM/small beam8/small beam1. `semantic_correction_required=true` означает необходимость смысловой правки цели, операции, объекта, роли, отрицания/отмены, условия, сущности, значения или ограничения. Косметика и ясная словоформа сами по себе не требуют правки; потерянные факты нельзя восстанавливать догадкой. Все семь измерений и точные причины ошибок сохраняются; null/unresolved не PASS. У выбранного варианта число таких случаев не выше CURRENT отдельно на dev16 и holdout16. Каждый новый hypothesis повторов также проверяется, ошибки не скрываются за iteration0.

WER≤15%, CER≤8% отдельно all32 и holdout16, completeness, cold≤120s,4CPU/4GiB,один native slot,concurrency2,три warm passes и substantial gain остаются обязательными. При REPLACE нельзя допустить новых непокрытых рисков; ошибка модели в явно показанном черновике не становится ошибочным принятым решением. Принятый текст должен дать100% semantic parity и0 неправильных решений в B02; главная transform-пара проходит до реального ответа без смысловой коррекции. B02 выполняется независимо от завершения B01 в общем24turn/20min ledger.

Разрешена конечная квалификация одного из уже измеренных вариантов в существующих не обнуляемых ledger: small1200s (израсходовано129.151549), Giga1800s (194.916977). Новые настройки/модели/скачивания/облачный ASR не разрешены. Выбор и полный freeze предшествуют holdout; после его ошибки gold/rubric/config не подстраиваются.

---

## Исторический дизайн4 сентября (протокол2, старые FAIL)
