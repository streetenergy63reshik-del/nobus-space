...
- локальные strict Pydantic-модели draft v1 с trusted ingress integrity bindings; owner-bound network identity активна только для внутреннего control plane, поэтому модели ещё не являются выпущенным внешним wire protocol;
...
- fake-first Codex CLI process boundary с injected spawner и независимо live-проверенным Windows Job guard; реальный Codex не запускался.
- local durable text/voice vertical, который атомарно сохраняет Task/WorkerEvents, передаёт worker result трём injected verifier boundaries и ставит terminal status в outbox;
- stdlib SQLite для durable ingress/task checkpoints, ordered events и status outbox, подключённый к локальному restart/recovery runtime;
- owner-bound `/task` control flow с in-memory one-shot capability, durable `PENDING`, `/confirm`/`/cancel`, локальным fake-worker и Telegram status outbox.

Text и подтверждённый voice ingress соединены с Core, SQLite и outbox локальной fake-композицией. Terminal Task/event/outbox transitions атомарны; restart не повторяет незавершённый worker автоматически. Owner-bound authenticated Telegram polling поддерживает безопасные control-команды. Confirmed fake-task flow принят code-wired/offline и runner активирован, но owner network `/task` → `/confirm` → terminal status ещё не воспроизведён. Voice network path, live CLI worker и production deployment отсутствуют.
...
|---|---|---|---|
| Telegram transport gateway | разобрать update, нормализовать text/voice/callback | недоверенный parser | live owner-bound status control; `/task` code-wired/offline accepted, owner network task E2E pending; voice/callback path не активирован |
| Authenticated ingress boundary | allowlist actor/chat, replay guard, tenant binding, trusted envelope | доверенный | bot identity + exact owner binding проверяются server-side; network auth активна в ограниченном control plane |
| Voice service | принять ограниченные bytes, transcribe, выдать и единожды подтвердить actor-bound preview | provider недоверен; confirmation boundary доверенный | preview и in-memory confirmation wired в local runtime; live download/transcription отсутствует |
| Prompt enricher | предложить instruction/criteria | недоверенный LLM worker | отсутствует |
| Local durable vertical | доказать text/voice → Core → SQLite → fake worker → L1/L2/L3 → outbox | демонстрационный | `/task` запускает его только после owner-bound one-shot confirm; live Codex отсутствует |
...
| CLI/code worker | локальная работа в allowlist/worktree | ограниченно доверенный | fake-first adapter; Windows Job guard live-проверен на probe-child, реальный Codex отсутствует |
...
| Notification adapter | отдать статус/результат без секретов | внешняя запись | Telegram status sender wired к content-free outbox; lease/ACK/NACK сохраняются |
