...
Новый [maintenance-кандидат](../adr/0028-post-mvp1-power-transition-recovery.md) исправляет обработку истёкшей голосовой кнопки и добавляет доказанное согласование после гибридного выключения. До отдельного развёртывания действующим LIVE остаётся `0c9b778`; синтетические тесты кандидата не являются production-проверкой. Историческая приёмка MVP1 от 19 сентября сохраняется отдельно, MVP2 не начат.
