...
C5 ACCEPTED / PASS / PUBLISHED; C6 HOLD; MVP1 NOT READY; MVP2 HOLD.
...
## Решение владельца для C5

В одной задаче разрешены отдельный C5 worktree, код, тесты, документация и обновление прежнего DOCX; после PASS — push C5 ветки, PR и обычный merge без обхода защиты. Одно необходимое документальное уточнение после merge также разрешено.

Владелец принял RPO ≤24 ч, RTO ≤30 мин; backup ежедневно и перед изменениями, хранение 7 ежедневных и 4 еженедельных копий. Полный запуск с SDK/ASR пока не измерен; копии на том же ПК не защищают от потери диска. Расписание и удаление рабочих копий в C5 не включаются.

Временное окно в C5 не запускалось. Постоянный deploy, изменение scheduler, live restore, C6 и tag/release не входят в эту задачу. Следующий C6 требует отдельного решения владельца.

## Сохранённый исторический объём C4
