...
- локальные strict Pydantic-модели draft v1 с integrity bindings; без trusted ingress и persistence это ещё не внешний wire protocol;
...
Text ingress и CLI boundary соединены с Core только локальной fake-композицией. Voice/callback flow, постоянного хранилища, authenticated identity boundary, live CLI worker и production deployment нет.
