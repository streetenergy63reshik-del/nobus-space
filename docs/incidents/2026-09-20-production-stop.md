...
## Исправление и отдельное развёртывание

По разрешению владельца исправление принято [PR №38](https://github.com/streetenergy63reshik-del/nobus-space/pull/38). Проверенный кандидат `863ba6f097199cbe8136516ab6ac16f389f76ed3` и merge/LIVE `9065cfc6a5b6e23fc44062e8a4d7001315033fa8` имеют одинаковое дерево. [ADR 0028](../adr/0028-post-mvp1-power-transition-recovery.md) описывает доказательство перехода питания, callback-проверку и совместимость v5.

Повторный разбор выявил и исправил `AttributeError` rollback-ридера на JSON не-объекте. В общем регрессионном наборе 305 проверок прошли; ошибочное предположение нового теста о первой ротации исправлено, после чего отдельно прошла 306-я проверка чтения v5, сжатого checkpoint, повреждённого JSON и rebind. Ранний единичный отказ чтения синтетической SQLite не повторился при диагностике и в общем прогоне; его причина неизвестна. Проверка текущих production-данных кандидатом прошла, активная lease правильно оставила запрет согласования. Секреты и сырые payload не включались в публикацию.

| Время МСК | Действие и подтверждённый результат |
|---|---|
| 16:16 | До изменения production сохранены точные XML трёх заданий, launcher, backup config и хэши строк БД; действующая квитанция backup complete, readiness PASS. |
| 16:17 | Admission hold, штатный STOP, отсутствие Core/relay/listener и активной lease подтверждено. Оператор ошибочно повторно захватил Core mutex перед штатным backup: `RunnerAlreadyActive`, LIVE ещё не переключён. |
| 16:20–16:21 | Подлинное частичное поколение содержало только `intent.dpapi`, без БД. Штатный `recover_partial` сохранил его в карантине. После исправления оператора создана и проверена новая prechange-копия; рабочие данные не изменились. |
| 16:22–16:23 | Чистый LIVE переключён на `9065cfc`, задания установлены disabled с точными прежними digest, новая backup config проверена, выполнен штатный exact-head rebind под production pythonw. |
| 16:24–16:26:57 | Запущен один штатный backup/start-цикл новой версии. Связанная копия VERIFIED, квитанция complete, admission hold снят, Main запущен, Health включён. |
| 16:27:36 | Независимый read-only срез PASS: четыре БД, cross-database, история, lease, local/public HTTP 200 за 234/766 мс, данные сохранены. Одна цепочка supervisor/Core, один relay, один listener; дети в Windows Job. |
| 16:28:05, 16:29:05 | Два различных аутентифицированных Health PASS по расписанию. Health и Backup имеют последний результат 0; Bot Running. |

Итоговая копия: `daily-20260920T162518-cf3c5d5b1fed4b47891fc44f4403c386`. Все строки трёх прикладных БД из остановленного baseline сохранены; четвёртая checkpoint-БД исправна, offset монотонен (`375633655` в итоговом срезе), lease активна. Pending/leased/unknown/failed deliveries — 0; confirmed parts — 20. История не удалялась, отката БД не было. Контролируемая пауза составила около 10 минут. Новое реальное сообщение в Telegram после переключения не отправлялось; text/TXT-проверка в 13:44 относится к восстановлению предыдущей версии.

Реальное выключение Windows ради проверки не выполнялось. Доказательство нового same-boot пути основано на регрессиях и чтении реальной пары System-событий на историческом времени; отсутствие доказательства или wake старше 15 минут сохраняет STOP. Это отдельный эксплуатационный ремонт, не повторная приёмка MVP1 и не запуск MVP2. Следующий штатный backup назначен на 21 сентября 03:30; неизвестный исход внешнего действия сначала проверяется, а не повторяется.
