...
- локальные strict Pydantic-модели draft v1 с известными security gaps; это ещё не wire protocol;
- L1/L2/L3 и L4 guards;
- один rule-based/AuditAgent прототип.

В изолированной ветке существуют Telegram parser/gateway и voice service preview. Они не интегрированы с Core. Постоянного хранилища, authenticated identity boundary, tenant-bound runtime Task, реального CLI worker и production deployment нет.
...
|---|---|---|---|
| Telegram transport gateway | разобрать update, нормализовать text/voice/callback | недоверенный parser | отдельная ветка |
| Authenticated ingress boundary | allowlist actor/chat, replay guard, tenant binding, trusted envelope | доверенный | отсутствует |
| Voice service | ограниченно принять stream, temp file, transcribe, очистить | provider недоверен | отдельная ветка |
...
| State/audit store | транзакционное состояние и append-only evidence | доверенный | in-memory прототип |
| CLI/code worker | локальная работа в allowlist/worktree | ограниченно доверенный | отсутствует |
...
CURRENT runtime автомат имеет 17 статусов. TARGET добавляет `executing` для внешнего эффекта после L4; точные переходы описаны в [`05-Спецификации-контрактов.md`](05-Спецификации-контрактов.md). Статус не приходит «готовым» от worker: WorkerEvent является входом, а Core решает переход.

Terminal: `completed`, `failed`, `escalate`. `rejected` может перейти только в `rework`; `deferred` — обратно в выполнение. После terminal запись результата, evidence и approval audit-locked.
...
- CURRENT Pydantic v1 — переходный контракт: в нём отсутствует часть binding полей TARGET. Расхождение исправляется до постоянного хранилища, без ложного объявления текущей схемы полной.
...
1. принимает только разрешённый размер и тип;
2. читает stream с hard limit;
...
7. очищает temp file при успехе, ошибке и cancellation;
