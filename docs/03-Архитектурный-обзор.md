...
| CLI/code worker | локальная работа в allowlist/worktree и server-owned owner library | ограниченно доверенный | реальный Codex CLI: обычные ответы и owner index выполняются tool-less; public research получает только model inference и строго валидированные web-search events, без shell/local-file/apps/MCP; owner library доступна только trusted server-side path index/file-transfer без передачи root или содержимого, отдельный spawner allowlist, bounded process/output/deadline и Windows Job; write-профиль изолирован отдельно |
