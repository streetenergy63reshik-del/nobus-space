...
**Состояние реализации:** CURRENT LOCAL OWNER RUNTIME; production-схема обозначена TARGET
...
- owner file adapter отправляет проверенные `.docx/.htm/.html/.pdf/.xlsx`
  до 50 MiB, не раскрывая абсолютный путь или sensitive names;
...
| Voice service | принять ограниченные bytes, transcribe, выдать и единожды подтвердить actor-bound preview | provider недоверен; confirmation boundary доверенный | live local download/transcription/preview; pending capability durable в `telegram-state.sqlite3` с TTL и one-shot claim |
...
| Nobus Core | contracts, policy, state, idempotency, routing, completion | доверенный | MVP-границы реализованы; accepted runtime state durable, policy registry загружается из server configuration |
...
| CLI/code worker | локальная работа в allowlist/worktree и server-owned owner library | ограниченно доверенный | реальный Codex CLI; read-only sandbox, отдельный `owner.library.read`, bounded process/output/deadline, Windows Job; OS-enforced read projection остаётся TARGET |
| Product effects | документы, download, network command и file delivery | внешняя запись | exact proposal + durable L4 capability + receipt; unknown network effect не повторяется |
...
CURRENT confirmation store принимает только точную пару `VoiceMessage` + `TrustedIngressEnvelope` + `VoicePreview`, выдаёт opaque capability с TTL и хранит только её SHA-256 digest. Gateway атомарно claim-ит capability, после чего confirm повторно связывает tenant, actor identity/role, auth context, user/chat, ingress revision/content ref, audio и transcript digest. Успех единственный: active binding заменяется минимальным tombstone без transcript/raw token. Store wired в live owner-bound Telegram control plane; после подтверждения transcript запускает тот же read-only Codex path. Store durable в `telegram-state.sqlite3`: незавершённый preview сохраняется после restart; UX TTL запрещает поздний запуск, а ограниченный recovery-retention позволяет доказанно отменить Core-задачу после краткого сбоя. One-shot claim и tombstone предотвращают повторное выполнение.
