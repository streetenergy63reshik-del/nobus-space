...
async def _run() -> None:
    ...
    try:
        ...
        await api.configure_profile(
            name="Nobus Space",
            description=(
                "Личный Telegram-интерфейс Nobus Space: принимает текстовые и "
                "голосовые задачи, показывает состояние и доставляет результат."
            ),
            short_description="Задачи, статусы и результаты Nobus Space.",
            commands=_COMMANDS,
        )
    finally:
        await api.aclose()
