...
**14 сентября 2026, 21:17 МСК:** MVP1 v1.0.2 опубликован; последний production-срез 15:34–15:37 МСК — DOWN. Пятый frozen candidate `5b74368` отклонён после собранного review. Пакет ремонта кода `23e47414932e236824c0d48430addac3cb4e9f88` имеет 137 целевых и 56 зависимых PASS; итоговый freeze, real fixture и независимые L1/L2/L3 впереди. Production не менялся. [Текущий статус](handoffs/CURRENT-STATUS.md) и [пакет M1-S1](gates/mvp1-maintenance/HANDOFF.md) содержат точные доказательства.
...
В принятом v1.0.2 Health не создаёт второй runtime, а legacy Scheduler retries не считаются recovery. Текущий maintenance WIP переносит ограниченные повторы внутрь одного Scheduler action: максимум 10 retries через 60 секунд, только для exact allowlisted transient outcome и после proven cleanup; permanent, `UNKNOWN`, invalid/missing или противоречащий exit code Core outcome, повреждённая evidence history или exhaustion дают typed STOP. Пять frozen revisions отклонены. До PASS real three-case Scheduler fixture, L1/L2/L3, publication и activation это TARGET, а не действующее поведение.
...
Production-задачи заменяются после экспорта exact XML main/health/backup и hashes config, admission hold, штатного STOP и доказанного отсутствия exact процессов/Job/lease/listener. Отключаются только эти три задачи. Installers с `-ReplaceExisting -StageDisabled` проверяют прежние definition/launcher digests, сохраняют rollback bytes и читают обратно disabled profile; промежуточный отказ оставляет задания отключёнными. Staged backup получает ближайшие будущие 03:30. Initialize/rebind/inspect/acknowledge принимают все три disabled задания с теми же signatures под production mutex; обычный run отклоняет disabled profile. После точного readback и initialize/rebind задания включаются, выполняется контролируемый candidate-bound backup cycle при сохранённом admission hold; он обеспечивает quiescence, свежую generation и разрешённый запуск одного runtime. До успешных backup/readiness/baseline приём закрыт. Fresh LIVE inputs обязательны перед activation. Возврат code/config/task bytes — только из экспортированных точных данных по решению владельца; БД поверх новых accepted tasks не откатываются.
