...
**Актуально на 9 сентября 2026.** Квалификационный экземпляр C6 остановлен после ошибки supervisor; причина пока неизвестна. [Текущий статус](handoffs/CURRENT-STATUS.md) содержит активную ревизию, состояние Планировщика и оставшиеся условия. Исправление промптов опубликовано, но ещё не включено; автоматическая проверка разрешений отклонила новый переход до отдельного подтверждения.
...
Сохранён квалификационный профиль: `NobusSpaceBot` остановлен, logon trigger отключён и без автоматических повторов; `NobusSpaceBot-Health` включён; `NobusSpaceBot-Backup` отключён после успешной реальной проверки. В расписании задано 03:30 МСК, но при Disabled следующего автоматического копирования нет.
...
Принятые цели: RPO ≤24 ч, RTO ≤30 мин; копия ежедневно 03:30 МСК и перед изменениями; хранение 7 daily / 4 weekly. После итоговой приёмки обязателен новый фактический post-accept backup с проверкой следующего запуска и возврата единственного runtime. До его успешного завершения постоянное резервирование не объявляется действующим.
