...
Text и подтверждённый voice ingress соединены с Core, SQLite и outbox локальной fake-композицией. Terminal Task/event/outbox transitions атомарны; restart не повторяет незавершённый worker автоматически. Owner-bound authenticated Telegram polling работает live только для безопасных control-команд; task execution, voice network path, live CLI worker и production deployment отсутствуют.
...
|---|---|---|---|
| Telegram transport gateway | разобрать update, нормализовать text/voice/callback | недоверенный parser | live polling для owner-bound text control; voice/callback network path не активирован |
...
| Local durable vertical | доказать text/voice → Core → SQLite → fake worker → L1/L2/L3 → outbox | демонстрационный | интегрирован; live network control пока не запускает runtime/worker |
...
| Notification adapter | отдать статус/результат без секретов | внешняя запись | только injected fake sender с lease/ACK/NACK |
